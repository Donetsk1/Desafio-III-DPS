...
export default function ProtectedHomeScreen() {
  ...
  // Cuentas dinámicas (extraídas de las transacciones)
  const accounts = useMemo(() => {
    return Array.from(new Set(transactions.map((item) => item.account))).filter(
      Boolean,
    );
  }, [transactions]);
  ...
  const onSaveTransaction = async () => {
    ...
    if (!validateAccountNumber(payload.account)) {
      Alert.alert("Error", "El número de cuenta debe ser de 10 dígitos.");
      return;
    }

    // ✅ Validación de presupuesto por categoría (solo para gastos)
    if (form.type === "gasto") {
      const { remaining } = calculateBudgetUsage(payload.category);
      if (payload.amount > remaining) {
        Alert.alert(
          "Presupuesto insuficiente",
          `Para ${payload.category} solo te quedan $${remaining.toFixed(2)}. No puedes gastar $${payload.amount.toFixed(2)}.`,
        );
        return;
      }
    }
  ...
  const calculateBudgetUsage = (category: string) => {
    const budgetAmount = budgets[category as keyof typeof budgets];
    if (budgetAmount === undefined) {
      return { spent: 0, remaining: 0 };
    }

    const totalSpent = transactions
      .filter((item) => item.category === category && item.type === "gasto")
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.section}>
        ...
        <TextInput
          ...
          placeholder="Numero de cuenta (10 dígitos)"
          placeholderTextColor="#6b7280"
          style={styles.input}
          value={form.account}
          maxLength={10}
      ...
      <View style={styles.section}>
        ...
        {FIXED_CATEGORIES.map((category) => {
          const { spent, remaining } = calculateBudgetUsage(category);
          return (
